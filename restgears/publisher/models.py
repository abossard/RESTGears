...
from publisher.views import download_handler
...
class NewsEntry(BaseModel):
    ...
    thumb_image_url = models.CharField(max_length=255, null=True, blank=True)
    ...
    def save(self, force_insert=False, force_update=False, using=None):
        ...
        super(NewsEntry, self).save(force_insert, force_update, using)
...
class NewsImage(models.Model):
    ...
    newsentry = models.ForeignKey(NewsEntry, related_name='images');
    image = models.ImageField(upload_to='uploads/news', help_text='Select an image that is already resized for the iphone');
    orderindex = models.IntegerField(default=100, help_text='NewsImage will be ordererd according to its Order Index')
